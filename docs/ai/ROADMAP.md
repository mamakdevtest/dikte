...
| **T4.8 ◐** | F2 | Burn down the `except Exception` allowlist: each remaining site either reports its failure or is explicitly listed with a reason — *measured: **312 broad handlers, 84 report, 228 say nothing**, and all 247 are recorded: 225 with a reason **derived from their own guarded body** (87 "optional widget", 41 "optional import", 43 "value of the wrong shape", 17 "view refreshed after the fact", 10 "presentation that may not resolve", 9 "worker that is gone", 4 "clipboard restore that is best effort", 4 "device list the machine may not offer", 3 "absent file or row", 3 "lookup that is not there", 3 "teardown that is already done", 2 "a target the settings may not have", 2 "platform path not taken here") and **22 hand-written ones, each sitting in the source above the statement that stays silent**. The ratchet fails when a site's code no longer implies its recorded reason, when a reason is not a sanctioned one, when the pending set grows, and when a site that needs a hand-written reason does not carry one — the checker itself is checked against a throwaway tree, because a guard nobody has watched fail is a guess. **The burn-down earned its keep on the last pass**: `meeting.prune_audio` built the set of recordings that must never be pruned from `read_meetings()` and, when that read failed, fell back to an empty set — the guard switched off, not "nothing to protect", so every recording past the retention went, including the only copy of a meeting that is not finished. It now prunes nothing while the question cannot be answered and says why on stderr (`AssertionError: 0 != 1` on the red proof). The second slice found a deadlier one: a hand-edited `cleanup_prompt` that was not text reached `.strip()` in `Config.load()` and **raised — the application did not start at all**, with no window, no message and nothing to do about it. A non-string prompt is now ignored out loud, a settings file that is not an object says so, and the two handlers that used to guard those paths are deleted rather than kept: an untestable claim is not a guard. **And the sixth slice found the burn-down's best argument yet**: the report written for `settings_ui`'s tab-change guard fired on *every* settings window ever opened — `Qt.UniqueConnection` does not exist in PyQt6, so that `connect` raised every time and `except: pass` swallowed it. The unsaved-edits question had never been asked, and an edit could be lost in silence. One attribute away (`Qt.ConnectionType.UniqueConnection`), pinned by a test that changes a tab with a dirty window and asserts the question was asked. What is left: shrinking the 206*

**Where it stands, after eleven slices (2026-09-13).** 291 -> 228. Eight of the slices found
something real — a data-losing prune, a settings file that stopped the application from starting,
a hardware claim nothing had verified, a picker left deaf, twelve features that vanished without
a word, an unsaved-edits guard that had never run since it was written, a prompt the user believed
was saved, and buttons that did nothing. Two of those were found by *making a failure speak* and
reading what it said in the next run's output, which is the argument for the ratchet rather than
against it.

The remaining 228 are distributed as: **83** optional widget (row visibility, `polish`, icons —
paint), **40** optional import (tkinter and label fallbacks), **39** value of the wrong shape
(safe fallbacks, read and left alone), **22** unclassified with a hand-written `# reason:`, **14**
view refreshed after the fact, **10** presentation, and 20 spread over seven other classes. The
last four slices each found less than the one before, and the two largest classes are now paint:
reading them again would be counting for counting's sake. The next real step is not another slice
but a decision — the gates in §5 — and the ratchet stays as the guard that keeps the 228 from
growing silently. |
...
- **N7 — showing the panel resets the interface language. Fixed (2026-09-13).**
  `_reposition()` built a `cfg.Config` to read one value, and `Config.__init__` re-applies
  the stored `ui_language` to the whole process. This note used to call it "harmless in the
  running app, where the two agree"; it is not. With the settings window open and a language
  picked but not yet saved, the panel appearing put the stored one back. The fix does not
  touch `Config`, whose side effect every caller relies on (the tests and the CLI included):
  the panel is handed the `conf` the process already holds and reads the corner from that.
  Two tests in `tests/test_thinking.py` — one for the guard, one pinning the mechanism, so
  that if reading a config ever stops applying the language, the second test says so.
